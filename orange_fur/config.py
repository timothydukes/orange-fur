...
class Config:
    ...
    spectral: float = 0.0          # Phase 15: %% of sections drawing a
                                   # SPECTRAL field (true partials via p12);
                                   # 0 = pure lattice = pre-P15 behavior
    echo_spec: str | None = None   # Phase 17: TOML echo control surface
    subset_cat: dict | None = None # GUI phase: per-category subset %%
                                   # (generation param; must match for replay)
    tension: float = 0.0           # Phase 19: roughness-trajectory strength
                                   # 0 (default) = pre-P19 selection path
    dynamics: str = "default"      # Phase 21: dynamics preset (remix knob;
                                   # pure mapping of drawn values, no draws)
    morph: float = 0.0             # Phase 23: morph-joint admission scale
                                   # (remix knob; 0 = clean cuts only)
    state: str | None = None       # Phase 22: comma list of state consumers
                                   # (fields,density,register,air,rooms) or
                                   # "all"; None = off. Remix knob: the
                                   # state is drawn+integrated regardless
                                   # (fixed draws); --state chooses readers
    surface: float = 0.0           # Phase 20: SURFACE category target share
                                   # (%% of terminals). 0 (default) = six
                                   # categories, byte-identical solver path.
                                   # A GENERATION parameter: must match for
                                   # replay; tokens do not cross the flag.
