...
CAT_DUR: dict[Cat, tuple[float, float]] = {
    ...
    Cat.SURFACE: (1.5, 14.0),      # Phase 20: textures, longer than clicks,
                                   # shorter than swells
}
CAT_AMP: dict[Cat, tuple[float, float]] = {
    ...
    Cat.GONG:    (0.35, 0.90),
    Cat.SWELL:   (0.20, 0.60),
    Cat.SURFACE: (0.05, 0.20),     # surface noise is quiet by nature
}
CAT_SEND: dict[Cat, tuple[float, float]] = {
    ...
    Cat.SURFACE: (0.30, 0.80),     # textures sit partly in the room
}
CAT_SLEW: dict[Cat, tuple[float, float]] = {
    ...
    Cat.SURFACE: (0.10, 0.60),     # env class does the shaping; slew moderate
}
