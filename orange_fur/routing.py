...
def _reso_scan(cid: str) -> str:
    """k-rate scan of giReso (rows: t, d1..d4, c1..c4 -- degrees AND cents,
    Phase 15) -- the exact giRooms idiom. Each resonator instance keeps its
    own row cursor. kc*_ are cents residuals: spectral fields put targets
    off the lattice, and the tank must ring on the true frequency."""
    return f"""
  kd0_{cid} init 0
  kd1_{cid} init 0
  kd2_{cid} init 0
  ...
  kc3_{cid} init 0
  kri_{cid} init 0
  krt_{cid} table kri_{cid} * 9, giReso
  if krt_{cid} >= 0 && ktime >= krt_{cid} then
    kd0_{cid} table kri_{cid} * 9 + 1, giReso
    kd1_{cid} table kri_{cid} * 9 + 2, giReso
    kd2_{cid} table kri_{cid} * 9 + 3, giReso
    kd3_{cid} table kri_{cid} * 9 + 4, giReso
    kc0_{cid} table kri_{cid} * 9 + 5, giReso
    kc1_{cid} table kri_{cid} * 9 + 6, giReso
    kc2_{cid} table kri_{cid} * 9 + 7, giReso
    kc3_{cid} table kri_{cid} * 9 + 8, giReso
    kri_{cid} = kri_{cid} + 1
  endif"""
...
def _unit_code(u: Unit, cid: str, inl: str, inr: str,
  ...
  kcp{cid}v{v}  =  kcp{cid}v{v} * (2 ^ (kc{sel}_{cid} / 1200))
  ...
  kcp{cid}v{v}  cpstun 1, kix{cid}v{v}, giTun
  kcp{cid}v{v}  =  kcp{cid}v{v} * (2 ^ (kc{v}_{cid} / 1200))
  kht{cid}v{v}  linseg 0, 0.08, {p['port']:.3f}
  kcp{cid}v{v}  portk  kcp{cid}v{v}, kht{cid}v{v}
