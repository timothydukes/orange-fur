...
has to special-case an instrument:
   ...
   p13 detg   (Phase 16: CENTS GLIDE, a delta -- the detune travels from
               p12 to p12+p13 over the note, on the p11 curve. 0 = static
               detune, prior behaviour bit-exactly.)
...
giAir    =     903                          ; Phase 22 air table (t, mul)
...
; RatRev -- Phase 20, the REVERSE envelope class (B7): a rational rising
; curve over the whole note into an instant terminus -- the reversed-tape
; percept. ishape 0..1 sets how late the rise concentrates. i-RATE ARGS
; ONLY (the Phase 0 rule).
opcode RatRev, a, ii
  ishape, idur  xin
  idur   =  idur < 0.001 ? 0.001 : idur
  ishape =  ishape < 0 ? 0 : (ishape > 1 ? 1 : ishape)
  iexp   =  1.5 + 6.0 * ishape          ; late-rise exponent
  at     phasor  1 / idur
  at     limit   at, 0.000001, 0.999999
  aout   =  at ^ iexp                   ; monotone rise to ~1, cut at note end
  xout   aout
endop
...
instr 90
  if $AIR. <= 0 igoto skip
  ; PHASE 22: the state's o3 breathes the air bed -- giAir rows of
  ; (t, mul) stepped at section boundaries (all-ones when the consumer is
  ; off, so the path is identical without --state air)
  ktime  timeinsts
  kair   init 1
  kaidx  init 0
  kat    table  kaidx * 2, giAir
  if kat >= 0 && ktime >= kat then
    kair   table  kaidx * 2 + 1, giAir
    kaidx  =  kaidx + 1
  endif
  aswell  randi  0.5, 0.07, 2          ; slow amplitude drift
  aswell  =  (0.5 + aswell) * kair
  anL     pinkish  1
  anR     pinkish  1
...
instr 99
  ...
  if kt >= 0 && ktime >= kt then
    kfbsT  table  kidx * 5 + 1, giRooms
    kcutsT table  kidx * 5 + 2, giRooms
    kwidT  table  kidx * 5 + 3, giRooms
    klag   table  kidx * 5 + 4, giRooms
    kidx = kidx + 1
  endif
  ; PHASE 23 rooms crossfade: the row's lag drives portk toward the
  ; STEPPED TARGETS (separate k-vars -- feeding portk its own output
  ; back would compound the lag every cycle). 0 at cuts (floored to
  ; 5 ms: effectively the old step), w/4 at morph joints so the room
  ; settles across the seam. portk's internal state still starts at 0
  ; (the P14 lesson), so the first ~30 ms rise from silence-narrow to
  ; row values -- inaudible at piece start and noted in the PHASELOG.
  klag  =  klag < 0.005 ? 0.005 : klag
  kfbs  portk  kfbsT, klag
  kcuts portk  kcutsT, klag
