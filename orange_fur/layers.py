...
def section_room(nodes, lo: int, hi: int, tilt: float | None = None) -> Room:
    ...
    PHASE 22 (rooms consumer): `tilt` in [0, 1] rotates the preference among
    NEAR-TIE rooms (votes >= top - 1) -- clear majorities still win, and
    room changes stay boundary-quantized (the clean-cut rule holds; state
    tilts WHICH room, never WHEN). tilt=None = the original rule exactly."""
    ...
    if tilt is not None:
        near = []
        for n in nodes[lo:hi]:                 # earliest-node order, stable
            if votes[n.l3] >= top - 1 and n.l3 not in near:
                near.append(n.l3)
        if len(near) > 1:
            return ROOMS[near[int(tilt * len(near)) % len(near)]]
