...
# Phase 20: SURFACE included -- texture loops are idiomatic (approved design;
# one-line reversal if unwanted). Appending keeps prior rng.choice indices'
# distribution shifted ONLY when SURFACE instruments exist; the choice draw
# count is unchanged (stream layout stable), and at --surface 0 the loop
# CAT may name SURFACE but the catmap has no pool -> the P11 uncovered-
# category rule keeps source timbre. To avoid even that at --surface 0,
# score.py filters the choice against the live catmap.
LOOP_CATS = [Cat.PLUCK, Cat.PARTIAL, Cat.TCLOUD, Cat.CLOUD]
# Phase 20: with --surface on, texture loops are idiomatic; the choice list
# gains SURFACE. The list is CONDITIONAL on the flag (not appended
# unconditionally) because rng.choice over 5 vs 4 elements changes OUTCOMES
# for the same draw -- at --surface 0 the doctrine demands pre-P20
# distribution exactly. Draw count is one either way (stream layout stable).
LOOP_CATS_SURFACE = LOOP_CATS + [Cat.SURFACE]
...
def draw_plan(rng: random.Random, span: float = 60.0, surface: bool = False) -> LoopPlan:
    ...
    return LoopPlan(prob=rng.uniform(*PROB), cell=cell,
                    cat=rng.choice(LOOP_CATS_SURFACE if surface
                                   else LOOP_CATS), voices=voices,
    ...
    for v in range(plan.voices):
        ...
        while t0 + k * tv < t0 + run:
            ...
            for cn in plan.cell:
                ...
                if st >= t0 + run:
                    continue
                _li, _ld = fold(base_index + cn.degree)
                out.append(Event(
