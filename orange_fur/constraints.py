...
def solve_categories(sys: System, rng: random.Random,
                     relaxed: list[str],
                     surface_pct: float = 0.0) -> list[Terminal]:
    ...
    rejection sampling:
      ...
      SURFACE -> (Phase 20, only when surface_pct > 0) the clustered slice
                 AFTER partials/tclouds: dense occurrence clusters land on
                 cheap surface noise BY ASSIGNMENT -- the honest resolution
                 of the node~100 sameness problem (variety from the string's
                 category structure, not from routing tricks). At
                 surface_pct == 0 this function is byte-for-byte the
                 six-category P1 path: the SURFACE block adds no draws.
    """
    n = sys.n
    ...
    # ---- Phase 20: surface noise, flag-gated ---------------------------
    if surface_pct > 0:
        n_surface = min(max(0, len(clustered) - n_partial - n_tcloud),
                        max(1, round(surface_pct / 100.0 * n)))
        for t in clustered[n_partial + n_tcloud:
                           n_partial + n_tcloud + n_surface]:
            cats[t] = Cat.SURFACE
...
def solve(n: int, rng: random.Random, max_attempts: int = 6,
          surface_pct: float = 0.0) -> Solution:
    ...
    for attempt in range(max_attempts * (len(RELAXATIONS) + 1)):
        ...
        sys_.terminals = solve_categories(sys_, rng, relaxed, surface_pct)
