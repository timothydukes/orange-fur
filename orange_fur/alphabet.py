...
class Cat(IntEnum):
    ...
    SURFACE = 6     # Phase 20: surface noise -- crackle, dust, hiss, filtered
                    #   transients. Flag-gated: exists in a piece only when
                    #   --surface > 0 (a GENERATION parameter). Cheap by
                    #   construction; assigned the clustered occurrence slice
                    #   so density lands on it.
...
CAT_TO_INSTR = {
    ...
    Cat.TCLOUD: 5,
    Cat.SWELL: 6,
    Cat.SURFACE: 4,   # placeholder path only (catmap=None tests): reuse the
                      # CLOUD placeholder instrument -- SURFACE is
                      # microsound-natured and the placeholder orchestra
                      # predates it. Real runs bind via catmap.
}
