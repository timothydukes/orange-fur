...
class MacroPlan:
    ...
    def culled(self, t: float, rng: random.Random) -> bool:
        # PHASE 23 STREAM DISCIPLINE: the draw is UNCONDITIONAL. The old
        # form drew only on density-track off-slots, so anything that
        # moves slot boundaries (the morph period stretch) changed draw
        # COUNTS and desynced --morph values (the P19/P22 lesson yet
        # again, hiding inside a two-line method). On-slot draws are
        # discarded values; version-gated layout shift.
        u = rng.random()
