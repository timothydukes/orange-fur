...
class Instrument:
    ...
    partials: list | None = None   # PHASE 19: (freq ratio, amp) pairs, <=12,
                                   # amp-normalized. Explicit where the
                                   # template holds a literal partial list;
                                   # template-class default otherwise
                                   # (roughness.TEMPLATE_SPECTRA).
...
class Orchestra:
    ...
    def subset(self, pct: float, rng: random.Random,
               per_cat: dict | None = None) -> "Orchestra":
        """--subset: keep pct% of each category, AT LEAST ONE per category.
        GUI-phase extension: `per_cat` maps category NAME -> percentage,
        overriding the global pct for that category (--subset-cat). A
        GENERATION parameter like --subset itself: it must match for
        replay. Unspecified categories inherit the global value; draw
        counts are unchanged in shape (one sample per category), so the
        stream layout is stable."""
        keep: list[Instrument] = []
        for cat in Cat:
            p = (per_cat or {}).get(cat.name, pct)
            pool = self.by_cat(cat)
            k = max(1, round(len(pool) * p / 100.0))
...
instr {num}
  ...
  ; PHASE 16 -- p13 is CENTS GLIDE, a DELTA: the note's detune travels from
  ; p12 to (p12 + p13) cents over its duration, sharing the p11 curve with
  ; the degree glide (one frequency trajectory, one transeg). p13 = 0 gives
  ; idetr2 = idetr and Phase 15 behaviour bit-exactly. Delta encoding is the
  ; doctrine: an absolute end-cents p13 would make 0 mean "glide back to the
  ; lattice", silently changing every detuned note.
  idetr =  2 ^ (p12 / 1200)
  idetr2 = 2 ^ ((p12 + p13) / 1200)
  icps0 =  icps
  icps  =  icps * idetr
  icps  =  icps > sr / 2.2 ? sr / 2.2 : icps
  icps2 =  cpstuni(p4 + igl, giTun) * idetr2
...
def t_bank(num: int, rng: random.Random, grades: int) -> Instrument:
    ...
    for i in range(k):
        ...
        _parts.append((2.0 ** (deg / grades), w))
    ...
    tot = sum(w for _, w in _parts) or 1.0
    return Instrument(num, Cat.PARTIAL, "bank", cost=1.6 * k,
                      peak=1.0, code="\n".join(lines),
                      partials=sorted(((r, w / tot) for r, w in _parts),
                                      key=lambda t: -t[1])[:12])
...
def t_modal(num: int, rng: random.Random, grades: int) -> Instrument:
    ...
    _parts = [((i + 1) ** stretch, 1.0 / (i + 1) ** 0.5) for i in range(k)]
    _pt = sum(w for _, w in _parts)
    _parts = [(r, w / _pt) for r, w in _parts][:12]
...
def t_tcloud(num: int, rng: random.Random, grades: int) -> Instrument:
    ...
    _pt = sum(1.0 / n for n in range(1, k + 1))
    _parts = [(float(n), (1.0 / n) / _pt) for n in range(1, min(k, 12) + 1)]
...
def t_bankswell(num: int, rng: random.Random, grades: int) -> Instrument:
    ...
    _pt = sum(1.0 / (i + 1) ** 0.7 for i in range(k))
    _parts = [(2.0 ** (d / grades), (1.0 / (i + 1) ** 0.7) / _pt)
              for i, d in enumerate(degs)][:12]
  ...
  amix  =  amix * aenv""")
    ...
    return Instrument(num, Cat.SWELL, "bankswell", comp=True, cost=1.5 * k,
                      peak=0.95, code=code, partials=_parts)


# ---------------------------------------------------------------- generator

# ---------------------------------------------------------------- SURFACE
# Phase 20 (B6+B7): the surface-noise family. Synthesized, no samples;
# CHEAP BY CONSTRUCTION (cost 1-4 oscili units) so density is affordable
# where the solver assigns it. PITCH SEMANTICS (approved): the note's tuned
# frequency is a COLORATION CENTER -- resonant filtering tracks
# cpstuni(p4) * kgl (glide) with idetr (p12) folded in by the shared
# preamble -- not an oscillator fundamental. Each template draws one of the
# B7 envelope classes: burst (RatWin, the existing world), emergence
# (attack occupying 0.85-0.98 of the note, then hard stop), reverse
# (RatRev rise into an instant terminus).


def _surface_env(rng: random.Random) -> tuple[str, str]:
    """(env class name, csound line producing aenv). Drawn per instrument."""
    r = rng.random()
    shape = rng.uniform(0.3, 0.9)
    if r < 0.4:
        return "burst", "  aenv  RatWin  islew, p3"
    if r < 0.7:
        # emergence: RatComp with an extreme slew reading -- the swell
        # machinery pushed to arrival-then-cut
        frac = 0.85 + 0.13 * shape
        return "emergence", (f"  aenv  RatRev  {frac:.3f}, p3\n"
                             f"  aenv  =  aenv * aenv")
    return "reverse", f"  aenv  RatRev  {shape:.3f}, p3"


def t_crackle(num: int, rng: random.Random, grades: int) -> Instrument:
    """Logistic-map chaos at k-rate gating filtered noise. The chaos
    parameter is drawn: low = sparse pops, high = dense fry. Tuned-ish: a
    reson at the note's frequency colors the crackle."""
    lam = rng.uniform(3.62, 3.99)      # logistic chaos band
    dens = rng.uniform(0.15, 0.85)     # gate threshold -> pop density
    q = rng.uniform(4, 30)
    env_name, env_line = _surface_env(rng)
    lines = [_head(num, f"SURFACE crackle lam={lam:.3f} env={env_name} (generated)")]
    lines.append(f"""  kx    init  0.5
  kx    =  {lam:.4f} * kx * (1 - kx)
  kgate =  kx > {dens:.3f} ? 1 : 0
  an    rand  1
  an    reson  an, kcps, kcps / {q:.1f}, 1
  asig  =  an * kgate""")
    lines.append(env_line)
    lines.append("  amix  =  asig * aenv * iamp")
    lines.append(_pan())
    lines.append(_bus_out())
    return Instrument(num, Cat.SURFACE, "crackle", cost=2.0,
                      peak=1.0, code="\n".join(lines))


def t_dustpop(num: int, rng: random.Random, grades: int) -> Instrument:
    """Random impulse train through popping VCAs into a tuned reson -- the
    pointillist end. Impulses built from a jittered phasor comparator (core
    opcodes only; no dependence on the dust opcode's availability)."""
    rate = rng.uniform(3, 80)
    decay = rng.uniform(6, 24)
    q = rng.uniform(6, 40)
    env_name, env_line = _surface_env(rng)
    lines = [_head(num, f"SURFACE dustpop rate={rate:.0f}/s env={env_name} (generated)")]
    lines.append(f"""  kj    randi  0.45, {rate / 3.0:.3f}
  aph   phasor  {rate:.2f} * (1 + kj)
  apop  =  exp(-{decay:.2f} * aph)
  an    rand  1
  an    reson  an, kcps, kcps / {q:.1f}, 1
  asig  =  an * apop""")
    ...
    lines.append(_pan())
    lines.append(_bus_out())
    return Instrument(num, Cat.SURFACE, "dustpop", cost=2.5,
                      peak=1.1, code="\n".join(lines))


def t_hiss(num: int, rng: random.Random, grades: int) -> Instrument:
    """Band-filtered noise floor with slow independent amp wobble and
    occasional ticks -- the vinyl-surface end. Widest band, least pitched:
    the tuned center colors, the bandwidth is generous."""
    bw = rng.uniform(0.8, 2.5)         # bandwidth as multiple of center
    wob = rng.uniform(0.05, 0.5)
    tick = rng.uniform(0.5, 4.0)       # ticks per second
    env_name, env_line = _surface_env(rng)
    lines = [_head(num, f"SURFACE hiss bw={bw:.2f} env={env_name} (generated)")]
    lines.append(f"""  kw    oscili  0.35, {wob:.4f}, giSine
  an    rand  1
  an    butbp  an, kcps, kcps * {bw:.3f}
  kt    randi  0.5, {tick:.3f}
  atick rand  1
  atk   =  atick * (kt > 0.42 ? 0.6 : 0)
  asig  =  an * (0.65 + kw) + atk * 0.25""")
    lines.append(env_line)
    lines.append("  amix  =  asig * aenv * iamp")
    lines.append(_pan())
    lines.append(_bus_out())
    return Instrument(num, Cat.SURFACE, "hiss", cost=1.5,
                      peak=0.9, code="\n".join(lines))


def t_transwash(num: int, rng: random.Random, grades: int) -> Instrument:
    """Filtered transient: a noise burst through a butbp whose center
    SWEEPS from/to the note's tuned frequency (drawn direction and depth)
    -- the family's gesture-scaled member."""
    up = rng.random() < 0.5
    depth = rng.uniform(1.5, 4.0)
    bw = rng.uniform(0.3, 0.9)
    env_name, env_line = _surface_env(rng)
    lines = [_head(num, f"SURFACE transwash {'up' if up else 'down'} x{depth:.1f} env={env_name} (generated)")]
    ia, ib = ("icps", f"icps * {depth:.3f}") if up else (f"icps * {depth:.3f}", "icps")
    lines.append(f"""  actr  expseg  {ia}, p3, {ib}
  kctr  downsamp  actr
  kctr  =  kctr * kgl
  an    rand  1
  an    butbp  an, kctr, kctr * {bw:.3f}""")
    lines.append(env_line)
    lines.append("  amix  =  an * aenv * iamp")
    lines.append(_pan())
    lines.append(_bus_out())
    return Instrument(num, Cat.SURFACE, "transwash", cost=2.0,
                      peak=1.0, code="\n".join(lines))
...
TEMPLATES = {
    ...
    Cat.SURFACE: [t_crackle, t_dustpop, t_hiss, t_transwash],
}


def generate(rng: random.Random, grades: int, n_buses: int = 1,
             surface: bool = False) -> Orchestra:
    ...
    orch = Orchestra()
    for cat in Cat:
        if cat == Cat.SURFACE and not surface:
            # Phase 20: flag-gated. Skipping the category entirely at
            # --surface 0 keeps every prior draw byte-identical (SURFACE is
            # the LAST enum member, so nothing after it shifts either).
            continue
        base = BASE_NUM[cat]
        temps = TEMPLATES[cat]
