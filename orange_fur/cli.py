...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--morph", type=_ranged(0.0, 1.0), nargs="?",
                   const=0.5, default=0.0, metavar="X",
                   help="Phase 23: continuous-morph form family. X scales "
                        "how readily interior section boundaries become "
                        "MORPH JOINTS (rooms crossfade, ~90%% field common "
                        "tones, minute-scale glides, stretched macro "
                        "periods, composed overlap) instead of clean cuts; "
                        "bare --morph = 0.5; 0 = the clean-cut world "
                        "byte-for-byte. Windows scale with the piece. "
                        "Remix knob: all values share a token")
    p.add_argument("--state", type=str, nargs="?", const="all",
                   default=None, metavar="LIST",
                   help="Phase 22: slow dynamical macro-state (hidden "
                        "weather) consumers -- comma list from fields,"
                        "density,register,air,rooms, or 'all' (= bare "
                        "--state). The 4-D coupled-oscillator state is "
                        "drawn in every piece; this chooses who reads it. "
                        "Remix knob: all values share a token")
    p.add_argument("--dynamics", choices=["default", "quiet", "mid",
                                          "limited"], default="default",
                   help="Phase 21 dynamics preset: pure remapping of drawn "
                        "amplitudes (no draws -- a remix knob; one token, "
                        "four dynamic readings). quiet = everything down, "
                        "full range; mid = moderate level, moderately "
                        "narrowed range; limited = narrow range, flattened "
                        "accent contour, no limiter anywhere. Presets move "
                        "the normalization target unless --normalize is "
                        "given explicitly")
    p.add_argument("--surface", type=_ranged(0.0, 100.0), nargs="?",
                   const=20.0, default=0.0, metavar="PCT",
                   help="Phase 20: SURFACE category (crackle/dust/hiss/"
                        "filtered transients) as a first-class solver "
                        "category. PCT = target share of terminal symbols; "
                        "bare --surface = 20. Absent/0 = the six-category "
                        "solver byte-for-byte. GENERATION parameter: must "
                        "match for replay")
    p.add_argument("--tension", type=_ranged(0.0, 1.0), default=0.0,
                   metavar="X",
                   help="roughness-trajectory strength 0-1: sections draw a "
                        "tension target (sensory dissonance percentile of "
                        "the run's own orchestra) and field choice, "
                        "vocabulary, and note-level degree choice bend "
                        "toward it. 0 (default) = off; any two values "
                        "share a replay token")
    p.add_argument("--subset-cat", type=str, default=None, metavar="SPEC",
                   help="per-category subset percentages, e.g. "
                        "'PARTIAL=80,PLUCK=60,GONG=20'; categories not "
                        "named inherit --subset. Generation parameter: "
                        "must match for --replay")
    p.add_argument("--echo-spec", type=str, default=None, metavar="TOML",
                   help="echo control surface: timing families "
                        "(uniform/multitap/euclid/ramp), pitch-mode and pan "
                        "weights, rotation, gestures, second-order echoes. "
                        "Versioned schema; unknown keys rejected. No file = "
                        "the P9-P11 echo distribution. Any two specs share "
                        "a replay token")
    ...
    p.add_argument("--spectral", type=_ranged(0.0, 100.0), default=0.0,
                   metavar="PCT",
                   help="percent of sections drawing a SPECTRAL harmonic "
                        "field -- sum/difference-tone or virtual-fundamental "
                        "partial complexes, realized as true frequencies via "
                        "cents residuals (p12). 0 (default) = pure lattice, "
                        "the pre-P15 behavior. Shares a replay token with "
                        "any other setting: same piece, harmony on or off "
                        "the lattice")
    ...
    p.add_argument("--normalize", type=float, default=None, metavar="DBFS",
...
def _parse_subset_cat(spec: str | None) -> dict | None:
    """'PARTIAL=80,PLUCK=60' -> {'PARTIAL': 80.0, ...}, validated."""
    if not spec:
        return None
    ...
    for part in spec.split(","):
        ...
        if not part:
            continue
        if "=" not in part:
            raise SystemExit(f"orange-fur: bad --subset-cat entry {part!r} "
                             "(want NAME=PCT)")
        name, val = (x.strip() for x in part.split("=", 1))
        name = name.upper()
        if name not in names:
            raise SystemExit(f"orange-fur: unknown category {name!r} "
                             f"(known: {', '.join(sorted(names))})")
        try:
            pct = float(val)
        except ValueError:
            raise SystemExit(f"orange-fur: bad percentage {val!r}")
        if not (1 <= pct <= 100):
            raise SystemExit("orange-fur: --subset-cat percentages must be "
                             "1-100")
        out[name] = pct
    return out or None
...
def main(argv: list[str] | None = None) -> int:
    ...
    kw = dict(
        ...
        normalize=(-3.0 if args.normalize is None else args.normalize),
        draft=args.draft, seed=args.seed,
        sections=args.sections, cost_cap=args.cost_cap, echo=args.echo,
        echo_spec=args.echo_spec,
        subset_cat=_parse_subset_cat(args.subset_cat),
        tension=args.tension,
        surface=args.surface,
        dynamics=args.dynamics,
        state=args.state,
        morph=args.morph,
        fields=args.fields, spectral=args.spectral,
    ...
    t0 = time.time()
    sol = solve(cfg.nodes, rng, surface_pct=cfg.surface)
    ...
    orch = full.subset(cfg.subset, rng, per_cat=cfg.subset_cat)
    ...
    from . import echocfg
    ecfg = echocfg.load(cfg.echo_spec)
    if cfg.echo_spec:
        print(f"  echo spec  {ecfg.source}")
    events, smeta = graph_events(cfg, sol, rng, catmap=catmap, ecfg=ecfg,
                                 insts=orch.instruments)

    # ---- Phase 21: dynamics preset -- pure post-generation amp mapping
    # (zero draws; `default` is the identity/byte path). The preset moves
    # the normalization target unless --normalize was given explicitly.
    from . import dynamics as DY
    _dp = DY.DYNAMICS[cfg.dynamics]
    DY.apply_dynamics(events, _dp)
    if _dp.norm_db is not None and args.normalize is None:
        cfg.normalize = _dp.norm_db
    gen_s = time.time() - t0
    ...
    stats["air_rows"] = smeta.get("air_rows", [])
    ...
    if win is not None:
        ...
        stats["air_rows"] = trim_timed(stats.get("air_rows", []),
                                       win[0], win[1])
    ...
    if mac:
        ...
        if cfg.surface > 0:
            _nsurf = sum(1 for e in events
                         if e.echo == 0 and e.cat == int(_Cat.SURFACE))
            _nsrc = sum(1 for e in events if e.echo == 0) or 1
            print(f"  surface    {_nsurf} notes "
                  f"({100.0 * _nsurf / _nsrc:.0f}% of source; target "
                  f"{cfg.surface:g}% of terminals)")
        if cfg.morph > 0:
            _mj = smeta.get("morph", {})
            _jl = _mj.get("joints", [])
            if _jl:
                print("  morph      " + ", ".join(
                    f"joint @{t:.0f}s (w {w:.0f}s)" for _i, t, w in _jl)
                    + f"  [{_mj.get('moved', 0)} notes composed, "
                      f"{_mj.get('reglided', 0)} reglided]")
            else:
                print("  morph      no joints admitted this run "
                      f"(x={cfg.morph:g})")
        if cfg.state:
            from .state import parse_state as _ps
            _en = ",".join(sorted(_ps(cfg.state))) or "none"
            print("  state      [" + _en + "]  " + " | ".join(
                f"{m['section'][:3]}: o=" + "[" +
                " ".join(f"{x:.2f}" for x in m.get("state", [])) + "]"
                for m in mac))
        if cfg.tension > 0:
            print("  tension    " + " | ".join(
                f"{m['section'][:3]}: {m['tension']}"
                + (f" (ach {m['tension_ach']})"
                   if m.get("tension_ach") is not None else "")
                for m in mac))
        vs = [(m["section"][:3], m["varispeed"]) for m in mac
              if m.get("varispeed")]
        if vs:
            print("  varispeed  " + " | ".join(f"{a}: {b}" for a, b in vs))
