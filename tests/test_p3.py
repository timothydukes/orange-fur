...
def test_generation():
    ...
    # P20 AMENDMENT: SURFACE is flag-gated -- absent at default generate().
    # The default-orchestra checks cover the six always-on categories; the
    # SURFACE family has its own suite (test_p20).
    for cat in [c for c in Cat if c != Cat.SURFACE]:
...
def test_subset_binding():
    ...
    for pct in (10, 33, 100):
        ...
        check(f"subset: {pct}% keeps every category",
              all(len(sub.by_cat(c)) >= 1
                  for c in Cat if c != Cat.SURFACE),   # P20: flag-gated
