...
def test_engine():
    fold = lambda i: (fold_index(i, 60, 12), 0.0)  # P15 tuple contract
...
def test_e2e():
    ...
    check("e2e: --echo 0 score is smaller",
          ...
          (n_lines("/tmp/p9c.csd"), n_lines("/tmp/p9a.csd")))
    # P16 amendment: p13 (cents glide) joined the contract -- 13 p-fields
    check("e2e: score lines carry 13 p-fields",
          all(len(l.split()) == 14 for l in Path("/tmp/p9a.csd").read_text()
