...
Version 0.25.0 — the complete spectral/ambient arc (Phases 0-23). Tuning:
Werckmeister III "mimafip" by default, any Scala `.scl` accepted. Output:
48 kHz stereo float WAV (release renders are synthesized at 96 kHz and
decimated). A wxPython GUI front-end (`python3 -m orange_fur.gui`) wraps
the CLI without replacing it.

The system as of v0.25.0, beyond the core generator: deterministic REPLAY
tokens (`--replay`, one token = one piece; mix-level flags re-read the
same piece); a preview WINDOW (`--from/--to` minutes); score-domain echo
DELAY treatments with a versioned TOML spec (`--echo`, `--echo-spec`) and
per-section VARISPEED warp; protected TAPE-LOOP phasing processes; MOTIF
capture and sectional quotation; per-section HARMONIC FIELDS with
spectrally derived (sum/difference, virtual-fundamental) field families
(`--fields`, `--spectral`) and scale-tuned resonators; cents detune and
cents-glide p-fields; a roughness→rewriting TENSION system (`--tension`);
a seventh SURFACE-noise instrument category (`--surface`, a generation
parameter); DYNAMICS presets (`--dynamics default/quiet/mid/limited`, no
limiter anywhere); a hidden 4-D dynamical MACRO-STATE read by five
consumers (`--state fields,density,register,air,rooms`); and the
CONTINUOUS-MORPH form family (`--morph`) — section boundaries that
crossfade rooms, carry ~90% field common tones, decelerate, and compose
their joints instead of cutting.

- `README.md` — this file: installation, setup, and the flag map.
- `USERGUIDE.md` — every flag, organized as parameter territories to explore,
  with recipes and templates.
- `PHASELOG.md` — the devlog: development history phase by phase, with
  engineering findings, stream-discipline rules, and test amendments.
- `MANUAL_GUI_CHECKLIST.md` — visual checks for the GUI that automated
  tests can't cover.
...
Generation parameters (change the piece; must match for `--replay`):
`--nodes`, `--duration`, `--sections`, `--subset`, `--subset-cat`,
`--surface`, `--tuning`.

Remix knobs (re-read the SAME piece — a token replays under any values):
`--echo`, `--echo-spec`, `--fields`, `--spectral`, `--tension`,
`--dynamics`, `--state`, `--morph`, `--wetdry`, `--space`, `--air`,
`--normalize`, `--draft`, `--from`/`--to`.

Full syntax, ranges, and recipes: `USERGUIDE.md`.
...
A wxPython front-end to the same CLI (it shows the exact command it will
run). Four tabs — Generate (scrollable), Echo spec editor, render
Library, Advanced — with the live render log at the bottom. Requires
`pip3 install wxPython`; the CLI needs nothing beyond section 1.
...
recipe templates. `PHASELOG.md` is the devlog — read it to understand
why the system behaves as it does (replay discipline, the no-limiter
rule, protected processes, stream-safety findings).
